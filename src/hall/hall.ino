...
const char* topicInCtl = "hass/ctl/hall/#";  // change this to the outgoing messages
const char* topicInRefresh = "hass/refresh";

const String clientId = "hall";
...
//Buz buz(D3);
Swi swi(D4, D8);
Pir p[pirNum] = {D6, D13, D1, D2};
LED led(D0);
Relay light(D5), doorSwi(D3, true), doorVoice(D11);
Mode mode;
LightCtl lightCtl(&light, &led, &mode);
Sensor doorTelSwi(D10);

int pirCnt = 0; //计数区域内激活的pir个数
unsigned long int LastActTime = 0;
unsigned long int doorSwiLastActTime = 0;
const unsigned int doorSwiMaxExeTime = 800;
...
void loop() {
    ...
    // if tel open voice
    if (doorTelSwi.isValChange()) {
        client.publish(String("hass/snsr/"+clientId+"/doorTelSwi").c_str(), String(doorTelSwi.getVal()).c_str());
        if (doorTelSwi.getVal() == 1) {
            doorVoice.on();
        } else {
            doorVoice.off();
        }
    ...
    // doorSwi trigger
    if(doorSwi.isStateChange()){
        client.publish(String("hass/snsr/"+clientId+"/doorSwi").c_str(), String(doorSwi.getStatus()).c_str());
        if(doorSwi.getStatus()){
            doorSwiLastActTime = millis();
         }
    }
    if(doorSwi.getStatus()){
        if(doorSwiLastActTime < millis() - doorSwiMaxExeTime){
             doorSwi.off(); 
        } 
    }
    
    // doorVoice trigger
    if(doorVoice.isStateChange()){
        client.publish(String("hass/snsr/"+clientId+"/doorVoice").c_str(), String(doorVoice.getStatus()).c_str());
    }
  // buz trigger
...
void mqtt_door(const String& subject, const String& content){
    if(subject == String("hass/ctl/"+clientId+"/doorSwi")){
        if(content == "0"){
            doorSwi.off();
        }
        if(content == "1"){
    ...
    if(subject == String("hass/ctl/"+clientId+"/doorVoice")){
        if(content == "0"){
            doorVoice.off();
        }
        if(content == "1"){
